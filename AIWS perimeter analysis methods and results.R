...
#Match
filtered_df$fwi_raw<-FWI_raw$fwi_raw[match(filtered_df$Event_id, FWI_raw$Fire_ID)]
...
#Aspen ratio by CWD Z-scores
model1<-lm(FPE~cwd_ZScore, data=Aspen_LC_Ratio)
summary(model1)

#Aspen ratio by FWI z-score (fires lifetime)
model2<-lm(FPE~isi_ZScore, data=Aspen_LC_Ratio)
summary(model2)

#Other covariates tested in model above: fwi_raw, fwi_ZScore, isi_fire_mean, DOY
